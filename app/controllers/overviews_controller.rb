class OverviewsController < ApplicationController
 ...
 def index
   if params[:search].present?
     @feed_entries = FeedEntry.includes(:feed, :feed_entry_images).all.order("published DESC").search(params[:search], suggest: true, track: true, page: params[:page], per_page: 9)
