class OverviewsController < ApplicationController
 ...
 def index
   if params[:search].present?
     ...
     @feed_entries = FeedEntry.preload(:feed).where(feed_id: current_user.followees(Feed)).order("published DESC").includes(:feed_entry_images).page(params[:page]).per_page(9)
  end
