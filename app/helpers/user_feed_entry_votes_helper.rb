module UserFeedEntryVotesHelper
  ...
  def link_to_upvote(object)
    button_to 'like', vote_path(:id => object.id), remote: true, method: :post, class: 'btn btn-primary btn-sm top-buffer'
  end

  def link_to_downvote(object)
    button_to 'unlike', vote_path(:id => object.id), remote: true, method: :delete, class: 'btn btn-danger btn-sm top-buffer'
  end
